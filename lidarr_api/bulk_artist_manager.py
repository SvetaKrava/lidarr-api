#!/usr/bin/env python3
"""
Bulk Artist Management Module for Lidarr

This module provides functionality to perform bulk operations on artists in Lidarr:
...
def bulk_monitor_artists(
    client: LidarrClient, artist_ids: List[int], monitored: bool
) -> None:
...
def bulk_tag_artists(
    client: LidarrClient, artist_ids: List[int], tag_ids: List[int], add: bool = True
) -> None:
    ...
    try:
        ...
        for artist in artists:
            client.update_artist(artist["id"], artist)
...
def search_all_albums(client: LidarrClient, artist_ids: List[int]) -> None:
    ...
    try:
        for artist_id in artist_ids:
            client.search_artist_albums(artist_id)
            artist = client.get_artist(artist_id)
            print(
                f"Triggered album search for {artist['artistName']} (ID: {artist_id})"
            )
...
def export_artists(
    client: LidarrClient, output_file: str, format_type: str = "json"
) -> None:
...
Examples:
  ...
  %(prog)s list-by-tag --tag-name rock
    ...
    monitor_parser.add_argument(
        "--artists", required=True, help="Comma-separated artist IDs"
    )

    # Unmonitor command
    unmonitor_parser = subparsers.add_parser("unmonitor", help="Unmonitor artists")
    unmonitor_parser.add_argument(
        "--artists", required=True, help="Comma-separated artist IDs"
    )
