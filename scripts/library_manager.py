...
def list_wanted_albums(client: LidarrClient, page: int = 1, page_size: int = 20, sort_by: str = 'airDateUtc') -> None:
    """List wanted/missing albums."""
    try:
        result = client.get_wanted(page=page, page_size=page_size, sort_key=sort_by, sort_dir='desc')
        ...
        for album in records:
            artist_name = album.get('artist', {}).get('artistName', 'Unknown') if album.get('artist') else 'Unknown'
        ...
        print(f"Error listing wanted albums: {e}")
...
def search_wanted_albums(client: LidarrClient, limit: int = 10) -> None:
    ...
    try:
        ...
        print(f"Error searching wanted albums: {e}")
...
def list_quality_profiles(client: LidarrClient) -> None:
    ...
    try:
        ...
        for profile in profiles:
            ...
            cutoff = profile.get('cutoff', {}).get('name', 'Unknown') if profile.get('cutoff') else 'Unknown'
        ...
        print(f"Error listing quality profiles: {e}")
...
def list_metadata_profiles(client: LidarrClient) -> None:
    ...
    try:
        ...
        print(f"Error listing metadata profiles: {e}")
...
def view_queue(client: LidarrClient, page: int = 1, page_size: int = 20, include_unknown_artist_items: bool = False) -> None:
    ...
    try:
        ...
        print(f"Download Queue (Page {page}, showing {len(records)} of {total_records} total items):")
        print(f"{'Artist':<25} {'Album':<35} {'Status':<15} {'Progress':<10} {'Time Left':<12}")
        print("-" * 110)
        
        for item in records:
            artist_name = item.get('artist', {}).get('artistName', 'Unknown') if item.get('artist') else 'Unknown'
            album_title = item.get('album', {}).get('title', 'Unknown') if item.get('album') else 'Unknown'
            ...
            print(f"{artist_name:<25} {album_title:<35} {status:<15} {progress_str:<10} {str(time_left):<12}")
...
def remove_queue_item(client: LidarrClient, queue_id: int, remove_from_client: bool = True, blocklist: bool = False) -> None:
...
Examples:
  ...
  %(prog)s queue remove --id 123 --remove-from-client --blocklist
    ...
    list_wanted_parser.add_argument('--page-size', type=int, default=20, help='Items per page (default: 20)')
    list_wanted_parser.add_argument('--sort-by', default='airDateUtc', help='Sort field (default: airDateUtc)')
    
    search_wanted_parser = wanted_subparsers.add_parser('search', help='Search wanted albums')
    search_wanted_parser.add_argument('--limit', type=int, default=10, help='Number of albums to search (default: 10)')
    ...
    imports_subparsers = imports_parser.add_subparsers(dest='imports_command', help='Import commands')
    ...
    view_queue_parser.add_argument('--page-size', type=int, default=20, help='Items per page (default: 20)')
    view_queue_parser.add_argument('--include-unknown', action='store_true', help='Include unknown artist items')
    ...
    remove_queue_parser.add_argument('--remove-from-client', action='store_true', help='Remove from download client')
