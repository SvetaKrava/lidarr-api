#!/usr/bin/env python3
"""
Lidarr System Maintenance Script

This script provides system maintenance functionality for Lidarr:
...
def create_backup(client: LidarrClient) -> None:
    ...
    try:
        ...
        result = client.start_backup()
        print("Backup creation initiated successfully")
        print("Note: The backup process runs asynchronously. Check the backups list to see when it completes.")
...
def restore_backup(client: LidarrClient, backup_name: str) -> None:
    ...
    try:
        ...
        print("WARNING: This will restore Lidarr to the state when the backup was created.")
        ...
        result = client.restore_system(backup_name)
...
def view_blocklist(client: LidarrClient, page: int = 1, page_size: int = 20) -> None:
    ...
    try:
        result = client.get_blocklist(page=page, page_size=page_size, include_artist=True)
        ...
        print(f"Blocklist (Page {page}, showing {len(records)} of {total_records} total items):")
...
def system_health(client: LidarrClient) -> None:
    ...
    try:
        ...
        print(f"  Runtime: {status.get('runtimeName', 'Unknown')} {status.get('runtimeVersion', '')}")
        ...
        try:
            ...
            for disk in disk_space:
                ...
                if total > 0:
                    ...
                    print(f"  {path}: {free_gb:.1f} GB free of {total_gb:.1f} GB ({used_percent:.1f}% used)")
                else:
                    print(f"  {path}: Unable to determine disk space")
        except:
            print("\nDisk Space: Unable to retrieve disk space information")
...
Examples:
  ...
  %(prog)s health
    ...
    blocklist_subparsers = blocklist_parser.add_subparsers(dest='blocklist_command', help='Blocklist commands')
    
    view_parser = blocklist_subparsers.add_parser('view', help='View blocklist')
    view_parser.add_argument('--page', type=int, default=1, help='Page number (default: 1)')
    view_parser.add_argument('--page-size', type=int, default=20, help='Items per page (default: 20)')
    
    remove_parser = blocklist_subparsers.add_parser('remove', help='Remove blocklist item')
    remove_parser.add_argument('--id', type=int, required=True, help='Blocklist item ID')
